...
import {
  ...
  handleChangeFilter,
  handleClearFilter,
} from "app/slices/myuhlSlices/myuhlAllClients/myuhlAllClients";
      ...
      <Row>
        ...
        <Col xs={12}>
          <Content style={{ minHeight: "70vh" }}>
            ...
            <Row className={"mb-8"}>
              ...
              <Col xs={4}>
                <BaseInput
                  ...
                  width="100%"
                  placeholder={"Поиск по номеру телефона, ФИО или Email"}
                  margin={"0 5px 0 0"}
                />
              </Col>
                    ...
                    {!isEmpty(userList) ? (
                      userList.map((item, index) => (
                        <tr key={index + 1}>
                          ...
                          <td>{get(item, "name", "-")}</td>
                          <td>{get(item, "sendPostCount", "-")}</td>
                          <td>{get(item, "email", "-")}</td>
                          <td>{get(item, "phoneNumber", "-")}</td>
                          <td>{get(item, "deviceType", "-")}</td>

                          <td>
                            <ReactSVG
                              onClick={() =>
                                deleteClient(get(item, "id", null))
                              }
                              src={deleteIcon}
            ...
            {get(allClientsDate, "paginationData.totalPages", 1) > 0 && (
